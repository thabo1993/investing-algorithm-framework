...
description: >-
  Senior full-stack engineer who designs and builds a production-ready startup MVP
  from scratch. Use when the user wants to go from idea to a minimal-but-scalable
  working product: system architecture, file structure, DB schema, API + UI, and
  real code. Triggers: "build an MVP", "design the whole system", "startup from scratch".
