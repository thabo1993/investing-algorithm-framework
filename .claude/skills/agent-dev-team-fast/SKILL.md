...
description: >-
  Orchestrate a full engineering team (Architect → Engineer → Reviewer → Optimizer) on one
  task, entirely inside this Claude session — no external LLM. Each role runs as a subagent
  via the Agent/Task tool, hands its output to the next, and you assemble the final result.
  Use when the user wants a feature/product built end to end "like a real engineering team",
  or types /agent-dev-team-fast.
...
entire pipeline runs inside this Claude Code session using the built-in **Agent** tool to spawn
...
- Use the **Agent** tool, one role at a time, passing the previous role's output in the prompt so
