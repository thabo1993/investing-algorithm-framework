...
category: quality
description: >-
  Run a retrospective after an agent or pipeline finishes: capture what the user corrected, what
  was redone, and what worked — then write structured lessons that future runs read back. The
  feedback loop that makes the library self-improving without any external service. Triggers:
  /agent-retro, "reflect on that run", "what did we learn", "capture lessons".
